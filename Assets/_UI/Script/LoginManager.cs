...
{
    public InputField id;        // 유저 아이디 변수.

    public InputField password; // 유저 패스워드 변수.

    public Text notify;         // 중복이 있는지 검사.
    ...
    {
        if (!CheckInput(id.text, password.text))                // 만일 입력 검사에 문제가 있을시 함수를 종료.
            return;
        {
    ...
    {
        if (CheckInput(id.text, password.text))            // 만일 입력 검사에 문제가 있으면 함수를 종료. 
            return;
        {
            ...
            {
                SceneManager.LoadScene("");                 // 다음 씬 "" 을 로드한다.
            }
            else
            {
    ...
    bool CheckInput(string id, string pwd)                      // 입력 완료 확인 함수
    {
        if (id == "" || pwd == "")                              // 만일, 입력란이 하나라도 비어 있으면 유저 정보 입력을 요구한다.
        {
